...
import { addRoomData, getRoomData } from "./database/roomFunction.mjs";
...
io.on("connection", function (socket) {
        ...
        await getUserData({ username: username }).then(async (res2) => {
            await getUserData({}).then((res) => {
                res.forEach((e) => {
                    if (
                        e.username != username &&
                        e.username.length >= data.length &&
                        e.username.substring(0, data.length) == data &&
                        !Object.keys(res2[0].duos).includes(e.username) &&
                        !res2[0].recievedRequests.includes(e.username) &&
                        !res2[0].sentRequests.includes(e.username)
                    ) {
    ...
    socket.on("send-user-request", async (data) => {
        ...
        if ("sentRequests" in expData === false) expData["sentRequests"] = [];
    ...
    socket.on("accept-request", async (data) => {
        ...
        if (data[2]) {
            ...
            await addRoomData({
                roomID: expData.duos[data[1]],
                messages: [],
                participants: [data[0] ,  data[1]],
            });
        } else {
            var searchExpData = [];
            var d = data[3];
            var username = expData.username;
            await getUserData({}).then(async (res) => {
                res.forEach((e) => {
                    if (
                        e.username != username &&
                        e.username.length >= d.length &&
                        e.username.substring(0, d.length) == d &&
                        !Object.keys(expData.duos).includes(e.username) &&
                        !expData.recievedRequests.includes(e.username) &&
                        !expData.sentRequests.includes(e.username)
                    ) {
                        searchExpData.push([e.username, e.avatar]);
                    }
                });
            });
            socket.emit("search-user-global-response", searchExpData);
        }
        await addUserData(expData);
        ...
        io.to(expData.socketID).emit(
            "removeSingle-sentRequestList",
            data[0]
        );
        if (data[2]) {
            expData.duos[data[0]] = uuidv1();
            io.to(expData.socketID).emit("addSingle-duoList", {
                name: data[0],
                img: avatar,
            });
            await addRoomData({
                roomID: expData.duos[data[0]],
                messages: [],
                participants: [data[0] , data[1]],
            });
        }
    ...
    socket.on("get-duoList", async (data) => {
        ...
        await getUserData({ username: data }).then((res) => {
            if (res.length != 0) expData = res[0].duos;
        });
        if (expData != null) {
            await Promise.all(
                Object.keys(expData).map(async (e) => {
                    await getUserData({ username: e }).then((res) => {
                        expData[e] = [
                            active.includes(res[0].socketID),
                            res[0].avatar,
                        ];
    ...
    socket.on("get-chat-details", async (data) => {
        ...
        socket.emit("recieve-chat-details", {
            publicKey: key,
            messageList: chats,
        });
    });
    socket.on("sending-message", async (data) => {
        await getUserData({ username: data.sender }).then((res) => {
            getRoomData({ roomID: res[0]["duos"][data.reciever] }).then(
                async (res2) => {
                    let newD = res2[0];
                    newD["messages"] = [
                        ...newD["messages"],
        ...
        await getUserData({ username: data.reciever }).then((res) => {
            getRoomData({ roomID: res[0].duos[data.sender] }).then(
                async (res2) => {
                    let newD = res2[0];
                    newD["messages"] = [
                        ...newD["messages"],
                    ...
                    await addRoomData(newD);
                    io.to(res[0].socketID).emit("recieve-single-message", [
                        {
                            time: data.time,
                            sender: data.sender,
                            message: data.message[1],
            ...
            getUserData({ username: socketUsername[socket.id] }).then((res) => {
                Object.keys(res[0].duos).forEach((e) => {
                    getUserData({ username: e }).then((res2) => {
                        io.to(res2[0].socketID).emit(
                            "friend-disconnected",
                            socketUsername[socket.id]
                        );
                    });
                });
            });
